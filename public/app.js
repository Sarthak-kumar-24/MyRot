...
function setupEventListeners() {
  ...
  document.getElementById("videoForm").addEventListener("submit", async (e) => {
    ...
    if (id) {
      const existingImages = currentPreviewImages.filter((img) =>
        img.startsWith("http"),
      );
  ...
  document
    ...
    .addEventListener("change", () => fetchVideos(true));
  document
...
async function fetchVideos(reset = false) {
  ...
  try {
    ...
    console.error(err);
    if (reset)
      alert("Make sure the backend is running and MongoDB is connected.");
  }
...
window.toggleFavorite = async function (id, event) {
  // 1. Stop click from triggering image sliders or links
  if (event) event.stopPropagation();

  const btn = document.querySelector(`.heart-btn[data-id="${id}"]`);
  if (!btn) return;

  // 2. Find video in state
  const video = videos.find((v) => v._id === id || v.id === id);
  if (!video) return;

  // 3. OPTIMISTIC UI UPDATE (Instant feedback)
  const isCurrentlyFav = video.isFavorite;
  video.isFavorite = !isCurrentlyFav; // Toggle local state
  ...
  try {
    ...
    video.isFavorite = isCurrentlyFav;
    if (video.isFavorite) {
      btn.classList.add("is-favorite");
      btn.innerHTML = '<i class="fa-solid fa-heart"></i>';
    } else {
    ...
    alert("Could not update wishlist. Check connection.");
  }
};
...
function renderVideos(newBatch, reset) {
  ...
  newBatch.forEach((video, index) => {
    ...
    const favIcon = video.isFavorite
      ? "fa-solid fa-heart"
      : "fa-regular fa-heart";

    card.innerHTML = `
        <div class="card-img-wrapper" id="carousel-${id}" style="position: relative;">
    ${imagesHTML}
    ...
    <!-- E-Commerce style floating wishlist button -->
    <button class="heart-btn wishlist-overlay ${favClass}" data-id="${id}" onclick="toggleFavorite('${id}', event)">
        <i class="${favIcon}"></i>
    </button>
</div>
